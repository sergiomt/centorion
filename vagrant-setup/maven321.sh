...
if [ -d "/usr/local/maven" ]
	...
	cd /usr/local
	MAVEN_FILE="apache-maven-3.2.1"
	wget_and_untar http://www.mirrorservice.org/sites/ftp.apache.org/maven/maven-3/3.2.1/binaries/ $MAVEN_FILE-bin.tar.gz maven.tar.gz
	mkdir maven
	cd $MAVEN_FILE
	mv * ../maven
	cd ..
