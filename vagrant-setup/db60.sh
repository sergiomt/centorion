...
if [ -d "/usr/share/db-6.0.30" ]
	then

	echo "Berkeley DB 6.0 is already installed, nothing done!"

else

	source /vagrant/vagrant-setup/include.sh

	# Berkeley DB 6.0 Can't compile the Java bindings with Java 8
	# so install Java 7, compile and uninstall Java 7
	yum install -y java-1.7.0-openjdk-devel

	cd /usr/share
	wget_and_untar http://download.oracle.com/berkeley-db/ db-6.0.30.tar.gz
	cd db-6.0.30/build_unix
	../dist/configure --prefix=/usr --enable-java --enable-dbm --enable-cxx && make
