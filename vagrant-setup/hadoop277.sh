...
# Install Hadoop 2.7.7 from source
...
if [ -d "/usr/share/hadoop" ]
	...
	then
		...
		if [ -f "/vagrant/vagrant-setup/hadoop/native/2.7.7/libhadoop.so" ]
		then
				echo "Using precompiled native libraries in cache"
				cp -u $SETUP/hadoop/native/2.7.7/*.* /usr/local/lib
